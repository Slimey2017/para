...
| `src/screens/home.js` | Main PARA launcher with Games, Apps, ParaStore, Bear Home, Creator, Social, Settings, recent activity, download, controller, network, storage, temperature, VR-US, avatar, clock, and notifications. | JavaScript | Works with clearly labeled mock status data. | Service-driven widgets, Quick Resume, real thermal/storage/network state, personalization. |
| `src/screens/libraries.js` | Game/app libraries, ParaStore, Creator Mode, and the full illustrated Bear Home room. Bear Home uses spatial controller-focus hotspots over the artwork and a More drawer for Photos, Games/UGC, Cloud, and Trash. | JavaScript | All screens navigate; the room interaction works while file operations remain honest stubs. | Package manager, licenses, app sandboxing, indexed files, commerce, UGC moderation, developer SDK, and animated/layered Bear Home assets. |
...
PARA Home exposes the seven major sections:

`Games | Apps | ParaStore | Bear Home | Creator | Social | Settings`
