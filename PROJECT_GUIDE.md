...
| `apps/para-home/assets/para-home-dashboard.png` | The authoritative PARA Home composition: matte-black shell, purple orbital background, top navigation, five launcher cards, recent activity, system overview, and shortcuts. PARA needs it to establish the intended console identity immediately. | 1672×941 PNG illustration | Works as the interactive Home surface; transparent focus controls are aligned over its visual cards. Data shown in the image remains mock development data. | Replace embedded copy/status values with layered live UI, optimize delivery formats, add localization variants, and support runtime personalization. Used by `src/screens/home.js`. |
| `apps/para-home/styles.css` | PARA’s matte-black, white, purple, liquid/wave identity; exact 16:9 artwork stages; TV scaling; tiles; panels; loading; disabled/focus states; startup effects. | Modern CSS | Works, including uncropped Home/Bear artwork, responsive scaling, and reduced-motion modes. | Design tokens, local font assets, GPU performance budgets, localization stress tests, HDR/color calibration. |
...
| `src/screens/home.js` | Builds the interactive layer over the PARA Home dashboard art. It maps controller/keyboard/pointer focus to Home, Store, Creator, Settings, notifications, Quick Menu, Continue, Explore, Create, Community, System, recent activity, and all shortcuts. | JavaScript HTML templates + positioned semantic buttons | Works. Routes open real prototype screens; Calendar, Achievements, Help, and activity data explicitly remain placeholders. | Replace baked-in mock status art with service-driven widgets, Quick Resume, real thermal/storage/network state, personalization, and localization-safe layout layers. Talks to the shared router/focus system through `app.js`. |
