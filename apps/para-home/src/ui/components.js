...
export function topbar({ section = "PARA" } = {}) {
  ...
  return `<header class="topbar">${brand()}<div class="topbar__right"><button class="topbar-icon" data-route="network" aria-label="Network settings">⌁</button><time class="clock" data-clock>--:--</time>${accountQuickMenu(profile)}</div></header>`;
}
