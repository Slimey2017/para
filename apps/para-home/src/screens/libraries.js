...
export function bearHomeScreen() {
  const hotspot = ({ label, collection, x, y, w, h, action = "open-collection", autofocus = false, className = "" }) => `<button class="bear-hotspot ${className}" style="--x:${x}%;--y:${y}%;--w:${w}%;--h:${h}%" data-action="${action}" ${collection ? `data-collection="${collection}"` : ""} data-focus-label="${label}" aria-label="${label}" ${autofocus ? "data-autofocus='true'" : ""}></button>`;
  return `<section class="bear-home-room" aria-label="Bear Home file explorer"><div class="console-art-frame bear-home-room__frame"><img class="console-art-frame__image bear-home-room__art" src="./assets/bear-home-room.png" alt="A cozy wooden room with a television, shelves, record player, desk, door, storage nook, and the PARA bear" />${hotspot({ label: "Videos", collection: "videos", x: .5, y: 22.5, w: 18.2, h: 34, autofocus: true })}${hotspot({ label: "Discs", collection: "discs", x: 27.7, y: 14.5, w: 9.4, h: 40 })}${hotspot({ label: "Music", collection: "music", x: 60.2, y: 19.5, w: 15.1, h: 33 })}${hotspot({ label: "Documents", collection: "documents", x: 74.4, y: 32.5, w: 14.5, h: 31 })}${hotspot({ label: "External Drives", collection: "external", x: 88.5, y: 19, w: 10.5, h: 47 })}${hotspot({ label: "Downloads", collection: "downloads", x: 66.7, y: 70.2, w: 17.2, h: 27 })}${hotspot({ label: "Bear Home menu", x: 39.5, y: 45.5, w: 10, h: 27, action: "bear-menu", className: "bear-hotspot--bear" })}<div class="bear-controller-prompts"><span><b data-prompt="confirm">Enter</b> Open</span><span><b data-prompt="back">Esc</b> Back</span></div><aside class="bear-menu" data-bear-menu hidden><div class="bear-menu__head"><h2>Bear Home</h2><button data-action="bear-menu-close" aria-label="Close">×</button></div><button data-route="apps" data-autofocus="true">Apps</button><button data-route="storage">Storage</button><button data-route="settings">Settings</button><button data-route="home">PARA Home</button></aside></div></section>`;
}
