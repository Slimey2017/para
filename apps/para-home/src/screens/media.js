...
export async function activateMediaGallery() {
  ...
  try {
    ...
    host.innerHTML = `<div class="media-gallery-grid">${items.map((item) => {
      ...
      return `<article class="media-gallery-card" data-capture-id="${item.id}"><div class="media-gallery-preview">${preview}</div><div class="media-gallery-card__copy"><span>${item.type === "clip" ? "GAMEPLAY CLIP" : "SCREENSHOT"}</span><strong>${fmt.format(item.createdAt)}</strong><small>${label}</small><div class="media-gallery-actions"><button type="button" data-action="share-capture" data-share-target="system" data-capture-id="${item.id}">Share</button><button type="button" data-action="share-capture" data-share-target="youtube" data-capture-id="${item.id}">YouTube</button><button type="button" data-action="share-capture" data-share-target="facebook" data-capture-id="${item.id}">Facebook</button><button type="button" data-action="share-capture" data-share-target="chat" data-capture-id="${item.id}">Chat</button><button type="button" data-action="share-capture" data-share-target="phone" data-capture-id="${item.id}">Phone</button><button type="button" data-action="delete-capture" data-capture-id="${item.id}">Delete</button></div></div></article>`;
    }).join("")}</div>`;
  } catch (error) {
