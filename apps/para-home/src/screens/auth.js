...
export function accountSignInScreen() {
  ...
  return `<section class="screen login-screen">${livingBackground()}<header class="profile-select__top">${brand()}<time data-clock>--:--</time></header><main class="login-card para-account-card"><span class="avatar login-avatar">P</span><span class="eyebrow">PARA Account</span><h1>Sign in</h1><p class="account-auth-copy">Use your PARA Account for identity, cloud-ready services, purchases, friends, and future cross-device sync.</p>${createdCopy}<form class="account-auth-form" data-account-signin-form><label><span>Email</span><input type="email" autocomplete="email" inputmode="email" value="${escapeHtml(rememberedEmail)}" data-account-email data-autofocus="true" required /></label><label><span>Password</span><input type="password" autocomplete="current-password" minlength="8" data-account-password required /></label><button type="submit" class="action-button login-continue" data-action="account-signin-submit">Sign In</button></form><p class="account-auth-status" data-account-auth-status aria-live="polite"></p><div class="login-options">${listRow({ title: "Forgot Password", icon: "?", route: "account-recovery" })}${listRow({ title: "Create PARA Account", icon: "+", route: "account-signup" })}${listRow({ title: "Back", icon: "↻", action: "account-auth-back" })}</div></main>${hints({ context: false, options: false })}</section>`;
}

export function accountRecoveryScreen() {
  const known = knownParaAccount();
  const rememberedEmail = sessionStorage.getItem("para.account.recovery.email") || sessionStorage.getItem("para.account.signin.email") || known?.email || "";
  return `<section class="screen login-screen">${livingBackground()}<header class="profile-select__top">${brand()}<time data-clock>--:--</time></header><main class="login-card para-account-card"><span class="avatar login-avatar">↻</span><span class="eyebrow">PARA Account Recovery</span><h1>Reset password</h1><p class="account-auth-copy">Enter the email on your PARA Account. If an account exists, Supabase will send a secure recovery link.</p><form class="account-auth-form" data-account-recovery-form><label><span>Email</span><input type="email" autocomplete="email" inputmode="email" value="${escapeHtml(rememberedEmail)}" data-account-email data-autofocus="true" required /></label><button type="submit" class="action-button login-continue" data-action="account-recovery-submit">Send Recovery Link</button></form><p class="account-auth-status" data-account-auth-status aria-live="polite"></p><div class="login-options">${listRow({ title: "Back to Sign In", icon: "←", route: "account-signin" })}</div></main>${hints({ context: false, options: false })}</section>`;
}

export function accountResetPasswordScreen() {
  return `<section class="screen login-screen">${livingBackground()}<header class="profile-select__top">${brand()}<time data-clock>--:--</time></header><main class="login-card para-account-card"><span class="avatar login-avatar">✓</span><span class="eyebrow">PARA Account Recovery</span><h1>Choose a new password</h1><p class="account-auth-copy">Your recovery link was accepted. Set a new password for this PARA Account.</p><form class="account-auth-form" data-account-reset-form><label><span>New password</span><input type="password" autocomplete="new-password" minlength="8" data-account-new-password data-autofocus="true" required /></label><label><span>Confirm password</span><input type="password" autocomplete="new-password" minlength="8" data-account-new-password-confirm required /></label><button type="submit" class="action-button login-continue" data-action="account-recovery-complete">Update Password</button></form><p class="account-auth-status" data-account-auth-status aria-live="polite"></p></main>${hints({ back: false, context: false, options: false })}</section>`;
}
